...
class BleConnectionService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_CONNECT -> { address = intent.getStringExtra(EXTRA_ADDRESS); deviceName = intent.getStringExtra(EXTRA_NAME) ?: "Montre"; connect() }
    ...
    private fun connect() {
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) { status("permission_missing", "Permission Bluetooth manquante"); return }
        ...
        gatt?.close(); status("connecting", "Connexion à $deviceName…")
    ...
    private val callback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(g: BluetoothGatt, status: Int, state: Int) {
            if (state == BluetoothProfile.STATE_CONNECTED) {
                attempts = 0; status("connected", "$deviceName connecté")
                ...
                g.close(); if (gatt === g) gatt = null; status("disconnected", "$deviceName déconnecté — reconnexion automatique"); reconnect()
            }
        }
        override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
            if (status != BluetoothGatt.GATT_SUCCESS) return
            subscribe(g, HR_SERVICE, HR_CHAR); subscribe(g, BAT_SERVICE, BAT_CHAR); subscribe(g, WEIGHT_SERVICE, WEIGHT_CHAR)
    ...
    private fun subscribe(g: BluetoothGatt, serviceId: UUID, charId: UUID) {
        ...
        d.value = BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE; g.writeDescriptor(d)
    }

    private fun parse(uuid: UUID, b: ByteArray) {
        if (b.isEmpty()) return
        val type: String; val value: Double
        when (uuid) {
            HR_CHAR -> { val f = b[0].toInt() and 255; val hr = if ((f and 1) == 0) b.getOrNull(1)?.toInt()?.and(255) ?: return else if (b.size >= 3) (b[1].toInt() and 255) or ((b[2].toInt() and 255) shl 8) else return; type="heart_rate"; value=hr.toDouble() }
    ...
    private fun reconnect() {
        val delay=minOf(60000L,2000L*(1L shl minOf(attempts,5))); attempts++
        android.os.Handler(mainLooper).postDelayed({ if (address != null) connect() },delay)
    }
    private fun disconnect(clear:Boolean) { if(clear) address=null; gatt?.disconnect(); gatt?.close(); gatt=null; stopSelf() }
    override fun onDestroy(){gatt?.close();gatt=null;super.onDestroy()}
    override fun onBind(intent:Intent?):IBinder?=null
    private fun status(s:String,m:String){sendBroadcast(Intent("com.paceos.app.BLE_STATUS").setPackage(packageName).putExtra("status",s).putExtra("message",m).putExtra("name",deviceName));getSystemService(NotificationManager::class.java).notify(NOTIFICATION_ID,notification(m))}
    private fun notification(text:String):Notification=NotificationCompat.Builder(this,CHANNEL_ID).setSmallIcon(android.R.drawable.stat_sys_data_bluetooth).setContentTitle("Pace — Montre").setContentText(text).setOngoing(true).setCategory(NotificationCompat.CATEGORY_SERVICE).build()
    private fun createChannel(){if(Build.VERSION.SDK_INT>=26)getSystemService(NotificationManager::class.java).createNotificationChannel(NotificationChannel(CHANNEL_ID,"Connexion montre",NotificationManager.IMPORTANCE_LOW))}
}