...
class MainActivity : ComponentActivity() {
    ...
    @SuppressLint("SetJavaScriptEnabled") override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);reader=HealthConnectReader(this);scheduleBackgroundSync();ContextCompat.registerReceiver(this,watchReceiver,IntentFilter().apply{addAction("com.paceos.app.BLE_SCAN_RESULT");addAction("com.paceos.app.BLE_SCAN_STATE");addAction("com.paceos.app.BLE_STATUS");addAction("com.paceos.app.BLE_SAMPLE")},ContextCompat.RECEIVER_NOT_EXPORTED);webView=WebView(this).apply{settings.javaScriptEnabled=true;settings.domStorageEnabled=true;settings.allowFileAccess=false;settings.allowContentAccess=false;webViewClient=object:WebViewClient(){override fun onPageFinished(v:WebView?,url:String?){super.onPageFinished(v,url);deliverPendingQueue();deliverBleQueue()}};addJavascriptInterface(HealthBridge(),"PaceHealthConnect");addJavascriptInterface(BleBridge(),"PaceWatch");loadUrl(BuildConfig.PACE_URL)};setContentView(webView)}
