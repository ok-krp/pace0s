...
import { localAiSupported } from "@/lib/local-ai";
...
export function AiSettings() {
  ...
  const refresh = async () => {
    ...
    setValue(prefs);
    setProviders({ ...config, coach: { ...config.coach, source: local.coach ? "local" : config.coach.source }, build: { ...config.build, source: local.build ? "local" : config.build.source } });
  };
  ...
  const persistAgent = async (agentType: AgentType) => {
    const config = providers[agentType]; setSaving(agentType);
    try {
  ...
  const test = async (agentType: AgentType) => { const config = providers[agentType]; if (config.source === "local") { toast.success(localAiSupported() ? "✓ IA locale disponible" : "IA locale indisponible sur ce navigateur"); return; } const effectiveProvider = config.provider; setTesting(agentType); try { const result = await testProvider({ data: { provider: effectiveProvider, model: config.model, baseUrl: effectiveProvider === "custom" ? config.baseUrl || null : null, apiKey: keys[agentType] || null } }); if (result.ok) toast.success("✓ Clé valide"); else toast.error(result.message); } catch (error) { toast.error(error instanceof Error ? error.message : "Impossible de contacter le fournisseur"); } finally { setTesting(null); } };
  const fetchModels = async (agentType: AgentType) => { const config = providers[agentType]; if (config.source === "local") return; setLoadingModels(agentType); try { const models = await listModels({ data: { provider: config.provider, baseUrl: config.provider === "custom" ? config.baseUrl || null : null, apiKey: keys[agentType] || null } }); setModelOptions((current) => ({ ...current, [agentType]: models })); toast.success(`${models.length} modèles disponibles`); } catch (error) { toast.error(error instanceof Error ? error.message : "Impossible de récupérer les modèles"); } finally { setLoadingModels(null); } };
  const removeKey = async (provider: AiProvider) => { try { await deleteKey({ data: { provider } }); await refresh(); setKeys({ coach: "", build: "" }); toast.success("Clé API supprimée"); } catch (error) { toast.error(error instanceof Error ? error.message : "Impossible de supprimer la clé"); } };
