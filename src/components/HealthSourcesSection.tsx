...
declare global { interface Window { PaceAppleHealth?: AppleHealthBridge; webkit?: { messageHandlers?: { paceHealthKit?: { postMessage: (message: unknown) => void } } }; } }
  ...
  return (
    <details className="pt-3 border-t border-border group">
      ...
      <div className="pt-3 space-y-1">
        <SourceRow icon={<Activity className="size-4" />} title="Health Connect" description="Android · Xiaomi / Mi Fitness → Health Connect → Pace" status={healthConnectAvailable ? "Disponible" : platform === "android" ? "Application Android requise" : "Indisponible dans la PWA"} action="Synchroniser" onClick={connectHealthConnect} />
        <SourceRow icon={<Apple className="size-4" />} title="Apple Santé" description="iPhone · Apple Santé / HealthKit → Pace" status={healthKitBridgeAvailable ? appleSyncing ? "Synchronisation…" : "Disponible" : platform === "ios" ? "Application iOS native requise" : "Indisponible dans la PWA"} action="Configurer" onClick={connectAppleHealth} />
        <SourceRow icon={<Smartphone className="size-4" />} title="Mi Fitness" description="Pas de connexion privée Xiaomi : les données passent par les APIs officielles du téléphone." status={platform === "android" ? "Via Health Connect" : platform === "ios" ? "Via Apple Santé" : "Via l'application mobile"} action="Voir le parcours" onClick={() => toast.info("Mi Fitness : active les données tierces puis synchronise vers Health Connect sur Android ou Apple Santé sur iPhone.")} />
        <div className="border-t border-dashed border-border/70 pt-3 text-[11px] text-muted-foreground leading-relaxed"><strong className="text-foreground">Aucune donnée simulée.</strong> Une source n'est indiquée comme connectée que lorsqu'une passerelle native réelle est disponible.</div>
      </div>
    </details>
  );
}

function SourceRow({ icon, title, description, status, onClick, action }: { icon: React.ReactNode; title: string; description: string; status: string; onClick: () => void; action: string }) {
  return <div className="flex items-start gap-3 py-3 border-b border-border/60 last:border-b-0"><div className="size-9 rounded-lg bg-muted grid place-items-center shrink-0">{icon}</div><div className="flex-1 min-w-0"><div className="font-medium text-sm">{title}</div><div className="text-xs text-muted-foreground mt-0.5">{description}</div><div className="flex items-center gap-1.5 mt-2 text-[11px]"><CheckCircle2 className="size-3.5" /><span>{status}</span></div></div><Button size="sm" variant="secondary" className="rounded-xl shrink-0" onClick={onClick}>{action}<ExternalLink className="size-3.5 ml-1" /></Button></div>;
}
