          ...
          {order.map((key, i) => {
            ...
            return (
              <li key={key} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm">
                ...
                <Button variant="ghost" size="icon" className="size-7" disabled={i === 0} onClick={() => move(i, i - 1)} aria-label="Monter"><ArrowUp className="size-4" /></Button>
                <Button variant="ghost" size="icon" className="size-7" disabled={i === order.length - 1} onClick={() => move(i, i + 1)} aria-label="Descendre"><ArrowDown className="size-4" /></Button>
              </li>
            );
          })}
        </ul>
