...
function SmartCardBase({
  ...
  return (
    <>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {handleQuickAdd && (
              ...
              >
                ...
                {hasDirectQuick ? (metric.key === "routine" ? "Habitude" : "Session") : (quickLabel ?? "Ajouter")}
              </motion.button>
            )}
          </div>
        )}
      ...
      {hasDirectQuick && (
        <Dialog open={directQuickOpen} onOpenChange={setDirectQuickOpen}>
          <DialogContent className="max-w-md">
            {metric.key === "routine" && <DashboardQuickRoutine onDone={() => setDirectQuickOpen(false)} />}
            {metric.key === "focus" && <DashboardQuickWork onDone={() => setDirectQuickOpen(false)} />}
          </DialogContent>
        </Dialog>
      )}
    </>
