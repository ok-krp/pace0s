...
import { connectNativeWatch, disconnectNativeWatch, initNativeWatchBridge, isNativeWatchAvailable, requestNativeBluetoothPermissions, startNativeWatchScan, stopNativeWatchScan, type NativeWatchDevice } from "@/lib/native-watch-bridge";
...
const labels: Record<string, { label: string; unit: string; icon: typeof HeartPulse }> = {
  heart_rate: { label: "Fréquence cardiaque", unit: "BPM", icon: HeartPulse }, oxygen_saturation: { label: "SpO₂", unit: "%", icon: Wind }, temperature_c: { label: "Température", unit: "°C", icon: Thermometer }, cadence_rpm: { label: "Cadence", unit: "tr/min", icon: Ruler }, power_w: { label: "Puissance", unit: "W", icon: Zap }, battery: { label: "Batterie", unit: "%", icon: BatteryFull }, weight: { label: "Poids", unit: "kg", icon: Dumbbell },
};
...
export function WatchManager() {
  ...
  const [nativeDevices, setNativeDevices] = useState<NativeWatchDevice[]>([]);
  ...
  const saveMetric = useCallback(async (type: string, value: number, source: string, ts: string) => {
    ...
    if (type === "battery" || type === "weight") return;
  ...
  const disconnect = useCallback(() => { if (nativeModeRef.current) { disconnectNativeWatch(); setConnected(false); return; } if (timerRef.current != null) window.clearInterval(timerRef.current); timerRef.current = null; try { connRef.current?.server.disconnect(); } catch {} connRef.current = null; setConnected(false); }, []);

  const connect = useCallback(async () => {
    if (isNativeWatchAvailable()) {
      nativeModeRef.current = true; setPairing(true); setNativeDevices([]); requestNativeBluetoothPermissions();
      if (!startNativeWatchScan()) { setPairing(false); toast.error("Impossible de démarrer la recherche Bluetooth native."); }
      return;
    }
  ...
  useEffect(() => {
    ...
    const cleanupNative = initNativeWatchBridge(
      (sample) => { void saveMetric(sample.type, sample.value, sample.source, new Date(sample.ts).toISOString()); },
      (status) => { const isConnected = status.status === "connected" || status.status === "ready"; setConnected(isConnected); setPairing(status.status === "connecting" || status.status === "scanning"); if (status.name) setDevice((d) => ({ id: d?.id ?? status.name!, name: status.name! })); if (status.status === "connected" || status.status === "ready") toast.success(status.message); },
      (found) => { setNativeDevices((prev) => prev.some((d) => d.address === found.address) ? prev : [...prev, found]); },
      (scan) => { setNativeScanning(scan.status === "scanning"); if (scan.status === "finished") setPairing(false); },
    );
    return () => { cleanupNative(); if (timerRef.current != null) window.clearInterval(timerRef.current); timerRef.current = null; };
  }, [saveMetric]);
  ...
  return <div className="space-y-5">
    <Card className="glass-card"><CardHeader className="flex flex-row items-center gap-3"><div className="size-11 rounded-2xl bg-primary/10 grid place-items-center"><Watch className="size-5 text-primary" /></div><div className="flex-1"><CardTitle>Montre</CardTitle><p className="text-xs text-muted-foreground mt-1">BLE temps réel + données quotidiennes Xiaomi via Health Connect</p></div>{connected ? <span className="text-xs flex items-center gap-1 text-emerald-600"><CheckCircle2 className="size-3.5"/> Connectée</span> : <span className="text-xs flex items-center gap-1 text-muted-foreground"><XCircle className="size-3.5"/> Déconnectée</span>}</CardHeader><CardContent className="space-y-4"><div className="flex items-center gap-2 flex-wrap"><Button onClick={connect} disabled={pairing || connected} className="gap-2"><Bluetooth className="size-4"/>{pairing ? "Recherche…" : "Connecter une montre"}</Button>{connected && <Button variant="outline" onClick={disconnect} className="gap-2"><XCircle className="size-4"/> Déconnecter</Button>}{device && <span className="text-sm text-muted-foreground flex items-center gap-1"><Link2 className="size-3.5"/>{device.name}</span>}</div>
      {nativeModeRef.current && nativeDevices.length > 0 && !connected && <div className="rounded-2xl border p-3 space-y-2"><div className="text-xs font-medium">Appareils Bluetooth détectés</div>{nativeDevices.map((d) => <button key={d.address} className="w-full text-left rounded-xl border p-3 hover:bg-muted/40" onClick={() => { setPairing(true); connectNativeWatch(d.address, d.name); }}><div className="font-medium text-sm">{d.name}</div><div className="text-xs text-muted-foreground">{d.address}{typeof d.rssi === "number" ? ` · ${d.rssi} dBm` : ""}</div></button>)}</div>}
    ...
    <Card className="glass-card"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle>Données quotidiennes</CardTitle><p className="text-xs text-muted-foreground mt-1">Source synchronisée : {health.lastSource ?? "aucune"}</p></div><Button size="sm" variant="outline" onClick={() => void refreshHealth()} className="gap-2"><RefreshCw className="size-3.5"/> Actualiser</Button></CardHeader><CardContent><div className="grid grid-cols-2 md:grid-cols-4 gap-3"><div className="rounded-2xl border p-4"><Footprints className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.steps.toLocaleString("fr-FR")}</div><div className="text-xs text-muted-foreground">Pas</div></div><div className="rounded-2xl border p-4"><Flame className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.kcalActive}</div><div className="text-xs text-muted-foreground">kcal actives</div></div><div className="rounded-2xl border p-4"><Ruler className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{(health.distanceM / 1000).toFixed(2)} km</div><div className="text-xs text-muted-foreground">Distance</div></div><div className="rounded-2xl border p-4"><Moon className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{Math.floor(health.sleepMin / 60)}h{String(health.sleepMin % 60).padStart(2, "0")}</div><div className="text-xs text-muted-foreground">Sommeil</div></div><div className="rounded-2xl border p-4"><HeartPulse className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.restingHeartRate ?? "—"}</div><div className="text-xs text-muted-foreground">FC repos</div></div><div className="rounded-2xl border p-4"><Dumbbell className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.exerciseMin} min</div><div className="text-xs text-muted-foreground">Entraînements</div></div><div className="rounded-2xl border p-4"><Flame className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.kcalTotal || "—"}</div><div className="text-xs text-muted-foreground">kcal totales</div></div><div className="rounded-2xl border p-4"><HeartPulse className="size-4 opacity-60"/><div className="text-2xl font-semibold mt-2">{health.heartRate ?? "—"}</div><div className="text-xs text-muted-foreground">Dernier BPM</div></div></div><div className="mt-4 text-[11px] text-muted-foreground">Dernière donnée : {health.lastTs ? new Date(health.lastTs).toLocaleString("fr-FR") : "aucune"}. Les valeurs sont issues des sources réellement synchronisées et ne sont pas estimées par Pace.</div></CardContent></Card>
  </div>;
}
