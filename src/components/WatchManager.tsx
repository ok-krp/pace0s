...
export function WatchManager() {
  ...
  const saveMetric = useCallback(async (type: string, value: number, source: string, ts: string) => {
    ...
    try { await insert({ data: { samples: [{ ts, type: type as "heart_rate" | "oxygen_saturation" | "temperature_c" | "cadence_rpm" | "power_w", value, source }] } }); setLastSync(ts); } catch {}
  }, [insert]);
  const readAll = useCallback(async () => {
    const conn = connRef.current; if (!conn || !conn.server.connected) return;
    for (const item of conn.chars) { try { if (!item.char.readValue) continue; const v = await item.char.readValue(); const parsed = item.service.parse(v); if (typeof parsed === "number") await saveMetric(item.service.type ?? "battery", parsed, `ble:${conn.device.name ?? "watch"}`, new Date().toISOString()); } catch {} }
  }, [saveMetric]);
