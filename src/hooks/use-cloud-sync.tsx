...
function isSyncableKey(key: string) { return key.startsWith(PACE_PREFIX) && !key.startsWith("pace.__") && key !== DOMAIN_OUTBOX_KEY && !EXCLUDED.has(key); }
function localKeys(): string[] { return Object.keys(localStorage).filter(isSyncableKey); }
function deviceId() {
  try {
...
function readMeta(): Record<string, string> { try { return JSON.parse(localStorage.getItem("pace.__sync_meta") ?? "{}"); } catch { return {}; } }
...
export function useCloudSync() {
  ...
  const pushAll = useCallback(async () => {
    ...
    setStatus("syncing");
    try {
      const id = deviceId(), keys = localKeys(), meta = readMeta();
      ...
      for (const key of keys) {
        ...
        if (row && Date.parse(row.updated_at) >= Date.parse(localTimestamp)) {
          applyRemoteWrite(key, row.value, row.updated_at); meta[key] = row.updated_at; rejected++; continue;
        }
  ...
  const pullAll = useCallback(async (overwrite = true) => {
    ...
    try {
      ...
      for (const rawRow of data ?? []) {
        ...
        let localValue: unknown = null; try { localValue = JSON.parse(localStorage.getItem(key) ?? "null"); } catch {}
        if (encoded(localValue) === encoded(row.value)) { meta[key] = row.updated_at; continue; }
        applyRemoteWrite(key, row.value, row.updated_at); meta[key] = row.updated_at; n++;
      }
      writeMeta(meta); setStatus("ok"); setLastMessage(`${n} clé${n > 1 ? "s" : ""} restaurée${n > 1 ? "s" : ""}.`);
    } catch (e) { setStatus("error"); setLastMessage(e instanceof Error ? e.message : "Erreur de synchronisation"); }
  }, [user]);
