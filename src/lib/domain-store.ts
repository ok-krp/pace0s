...
function nutritionFingerprint(item: unknown) {
  if (!item || typeof item !== "object" || Array.isArray(item)) return "invalid";
  const x = item as Record<string, unknown>;
  return JSON.stringify({ name: x.name ?? "", meal: x.meal ?? "", kcal: Number(x.kcal ?? 0), p: Number(x.p ?? 0), c: Number(x.c ?? 0), f: Number(x.f ?? 0), fiber: Number(x.fiber ?? 0), sugar: Number(x.sugar ?? 0), sodium: Number(x.sodium ?? 0), qty: Number(x.qty ?? 1) });
}

/**
 * Nutrition is user-entered data, so identical rows can be legitimate. We only
 * collapse exact duplicates beyond a small safety threshold, and cap an
 * obviously corrupted runaway day. This is deliberately conservative.
 */
export function sanitizeNutritionItems(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) return value;
  const source = value as Record<string, unknown>;
  const output: Record<string, unknown> = {};
  for (const [day, rawList] of Object.entries(source)) {
    if (!Array.isArray(rawList)) { output[day] = []; continue; }
    const counts = new Map<string, number>();
    const cleaned: unknown[] = [];
    for (const item of rawList) {
      const fingerprint = nutritionFingerprint(item);
      const count = counts.get(fingerprint) ?? 0;
      if (count >= MAX_IDENTICAL_NUTRITION_ITEMS_PER_DAY) continue;
      counts.set(fingerprint, count + 1);
      cleaned.push(item);
    }
    output[day] = cleaned.length > MAX_NUTRITION_ITEMS_PER_DAY ? cleaned.slice(-MAX_NUTRITION_ITEMS_PER_DAY) : cleaned;
  }
  return output;
}
...
function recomputeNutritionTotals(value: unknown): Record<string, NutritionTotals> {
  ...
  for (const [day, rawList] of Object.entries(value as Record<string, unknown>)) {
    ...
    totals[day] = list.reduce<NutritionTotals>((a, raw) => { const x = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {}; return { kcal: a.kcal + Number(x.kcal ?? 0), p: a.p + Number(x.p ?? 0), c: a.c + Number(x.c ?? 0), f: a.f + Number(x.f ?? 0) }; }, { kcal: 0, p: 0, c: 0, f: 0 });
  }
  return totals;
}
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed.version === 1 && typeof parsed.updatedAt === "string" && typeof parsed.mutationId === "string" && "value" in parsed) {
        if (domain === "nutrition.items") parsed.value = sanitizeNutritionItems(parsed.value) as T;
        return parsed as DomainRecord<T>;
      }
    }
    const legacyRaw = localStorage.getItem(`pace.${domain}`);
    if (legacyRaw !== null) { try { const value = JSON.parse(legacyRaw) as T; return { version: 1, updatedAt: new Date(0).toISOString(), mutationId: "legacy-import", value: domain === "nutrition.items" ? sanitizeNutritionItems(value) as T : value }; } catch {} }
  } catch {}
...
export function writeDomain<T>(domain: string, value: T): DomainRecord<T> {
  const safeValue = domain === "nutrition.items" ? sanitizeNutritionItems(value) as T : value;
  const record: DomainRecord<T> = { version: 1, updatedAt: new Date().toISOString(), mutationId: mutationId(), value: safeValue };
  if (typeof window !== "undefined") {
    try {
      localStorage.setItem(storageKey(domain), JSON.stringify(record));
      localStorage.setItem(`pace.${domain}`, JSON.stringify(safeValue));
      window.dispatchEvent(new CustomEvent(WRITE_EVENT, { detail: { domain, record } }));
      window.dispatchEvent(new CustomEvent(LOCAL_WRITE_EVENT, { detail: { key: `pace.${domain}`, value: safeValue, updatedAt: record.updatedAt, mutationId: record.mutationId } }));
      if (domain === "nutrition.items") {
        const totals = recomputeNutritionTotals(safeValue);
        const totalsRecord: DomainRecord<typeof totals> = { version: 1, updatedAt: record.updatedAt, mutationId: mutationId(), value: totals };
        ...
        window.dispatchEvent(new CustomEvent(LOCAL_WRITE_EVENT, { detail: { key: "pace.nutrition.totals", value: totals, updatedAt: totalsRecord.updatedAt, mutationId: totalsRecord.mutationId } }));
      }
    } catch {
...
export function useDomainState<T>(domain: string, fallback: T): [T, (next: T | ((previous: T) => T)) => void] {
  ...
  useEffect(() => {
    const onWrite = (event: Event) => {
      ...
      setRecord((current) => compareRecords(detail.record, current) <= 0 ? current : detail.record);
    };
    const onRemoteWrite = (event: Event) => {
      const detail = (event as CustomEvent<{ key: string; value: T }>).detail;
      if (!detail || (detail.key !== `pace.${domain}` && detail.key !== `pace.domain.${domain}`)) return;
      const next = readDomain<T>(domain, fallback);
      setRecord((current) => compareRecords(next, current) <= 0 ? current : next);
    };
    window.addEventListener(WRITE_EVENT, onWrite);
    window.addEventListener(REMOTE_WRITE_EVENT, onRemoteWrite);
    return () => { window.removeEventListener(WRITE_EVENT, onWrite); window.removeEventListener(REMOTE_WRITE_EVENT, onRemoteWrite); };
  }, [domain]);
