...
export function readDomain<T>(domain: string, fallback: T): DomainRecord<T> {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed.version === 1 && typeof parsed.updatedAt === "string" && typeof parsed.mutationId === "string" && "value" in parsed) return parsed as DomainRecord<T>;
    }
    const legacyRaw = localStorage.getItem(`pace.${domain}`);
    if (legacyRaw !== null) { try { return { version: 1, updatedAt: new Date(0).toISOString(), mutationId: "legacy-import", value: JSON.parse(legacyRaw) as T }; } catch {} }
  } catch {}
...
export function writeDomain<T>(domain: string, value: T): DomainRecord<T> {
  const record: DomainRecord<T> = { version: 1, updatedAt: new Date().toISOString(), mutationId: mutationId(), value };
  if (typeof window !== "undefined") {
    try {
      ...
      if (domain === "nutrition.items") {
        ...
        const totalsRecord: DomainRecord<typeof totals> = { version: 1, updatedAt: new Date().toISOString(), mutationId: mutationId(), value: totals };
