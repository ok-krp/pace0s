...
export function readDomain<T>(domain: string, fallback: T): DomainRecord<T> {
  ...
  try {
    ...
    if (legacyRaw !== null) {
      try { return { version: 1, updatedAt: new Date(0).toISOString(), mutationId: "legacy-import", value: JSON.parse(legacyRaw) as T }; } catch {}
    }
  } catch {}
...
export function writeDomain<T>(domain: string, value: T): DomainRecord<T> {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      // The timestamp belongs to the user mutation. A remote application must
      // never manufacture a new timestamp for this event.
      window.dispatchEvent(new CustomEvent(LOCAL_WRITE_EVENT, { detail: { key: `pace.${domain}`, value, updatedAt: record.updatedAt, mutationId: record.mutationId } }));
    } catch {}
  }
  return record;
}
