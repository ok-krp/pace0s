...
/** Merge genuinely new Nutrition items. A remote snapshot may be incomplete because
 * legacy/local-only foods are not necessarily present in food_log. Never discard a
 * local item merely because it is absent from the remote snapshot. */
function mergeNutritionValues(localValue: unknown, incomingValue: unknown) {
  const unwrappedLocal = unwrapNutritionValue(localValue);
  const unwrappedIncoming = unwrapNutritionValue(incomingValue);
  if (!unwrappedLocal || typeof unwrappedLocal !== "object" || Array.isArray(unwrappedLocal) || !unwrappedIncoming || typeof unwrappedIncoming !== "object" || Array.isArray(unwrappedIncoming)) return null;
  const local = unwrappedLocal as Record<string, unknown>; const incoming = unwrappedIncoming as Record<string, unknown>; const merged: Record<string, unknown> = { ...local };
...
export function readDomain<T>(domain: string, fallback: T): DomainRecord<T> {
  ...
  try { const raw = localStorage.getItem(storageKey(domain)); if (raw) { const parsed = JSON.parse(raw) as Partial<DomainRecord<T>>; if (parsed.version === 1 && typeof parsed.updatedAt === "string" && typeof parsed.mutationId === "string" && "value" in parsed) { if (domain === "nutrition.items") parsed.value = sanitizeNutritionItems(unwrapNutritionValue(parsed.value)) as T; return parsed as DomainRecord<T>; } } const legacyRaw = localStorage.getItem(`pace.${domain}`); if (legacyRaw !== null) { try { const value = JSON.parse(legacyRaw) as T; return { version: 1, updatedAt: new Date(0).toISOString(), mutationId: "legacy-import", value: domain === "nutrition.items" ? sanitizeNutritionItems(unwrapNutritionValue(value)) as T : value }; } catch {} } } catch {}
...
export function writeDomain<T>(domain: string, value: T): DomainRecord<T> {
  const safeValue = domain === "nutrition.items" ? sanitizeNutritionItems(unwrapNutritionValue(value)) as T : value; const record: DomainRecord<T> = { version: 1, updatedAt: new Date().toISOString(), mutationId: mutationId(), value: safeValue };
...
export function useDomainState<T>(domain: string, fallback: T): [T, (next: T | ((previous: T) => T)) => void] {
  ...
  useEffect(() => {
    const onWrite = (event: Event) => {
      ...
      setRecord((current) => { if (domain === "nutrition.items") { const merged = mergeNutritionValues(current.value, detail.record.value); return merged ? { ...detail.record, value: merged as T } : current; } return compareRecords(detail.record, current) <= 0 ? current : detail.record; });
    };
    const onRemoteWrite = (event: Event) => {
      ...
      setRecord((current) => { if (domain === "nutrition.items") { const merged = mergeNutritionValues(current.value, next.value); return merged ? { ...next, value: merged as T } : current; } return compareRecords(next, current) <= 0 ? current : next; });
    };
