const ECDH_ALGORITHM = "ECDH-P256";
...
export async function exportHealthDevicePublicKey(
  key: CryptoKey,
): Promise<HealthE2eeDevicePublicKey> {
  return crypto.subtle.exportKey("jwk", key) as Promise<HealthE2eeDevicePublicKey>;
}

async function deriveWrappingKey(
  privateKey: CryptoKey,
  peerPublicKey: HealthE2eeDevicePublicKey,
): Promise<CryptoKey> {
...
/**
 * Wraps the non-extractable health master CryptoKey directly. The plaintext
 * health key never becomes JS-readable during pairing.
 */
...
export async function unwrapHealthKeyEnvelope(args: {
  ...
  return crypto.subtle.unwrapKey(
    ...
    {
      name: "AES-GCM",
      iv: fromBase64(args.envelope.nonce),
      additionalData: envelopeAad(
        args.envelope.deviceId,
        args.envelope.senderDeviceId,
        args.envelope.keyVersion,
      ),
    },
    { name: "AES-GCM", length: 256 },
    false,
