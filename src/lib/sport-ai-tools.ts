...
function logged<Input>(name: string, execute: (input: Input, options: ToolExecutionOptions<unknown>) => Promise<Output>) {
  return async (input: Input, options: ToolExecutionOptions<unknown>) => {
...
export function canonicalSportTools(client: Client, userId: string, permissionsEnabled: boolean, logAction: LogAction) {
  const denied = <Input,>() => logged<Input>("sport_permission", async () => ({ ok: false, message: "Permission Sport désactivée." }));
  const guard = <Input>(name: string, execute: (input: Input, options: ToolExecutionOptions<unknown>) => Promise<Output>) =>
  ...
  const createExercise = tool({
    description: "Créer un exercice Sport dans la base normalisée Pace.",
  ...
  const updateExercise = tool({
    description: "Modifier un exercice Sport existant appartenant à l'utilisateur.",
    inputSchema: exerciseSchema.extend({ id: z.string().uuid() }),
  ...
  const deleteExercise = tool({
    description: "Supprimer un exercice Sport existant. Refuser si la base empêche la suppression par des dépendances.",
  ...
  const createProgram = tool({
    description: "Créer un programme Sport avec ses exercices et objectifs.",
    ...
    execute: guard("create_program", async (input) => {
      ...
      if (input.items.length) {
        const { error: itemError } = await client.from("sport_program_items").insert(input.items.map((item, index) => ({ program_id: id, exercise_id: item.exerciseId, position: item.position ?? index, sets: item.sets, reps: item.reps, weight: item.weight ?? null, rest_sec: item.restSec ?? null })));
  ...
  const updateProgram = tool({
    description: "Modifier le nom, l'emoji ou les jours d'un programme Sport.",
    inputSchema: z.object({ id: z.string().uuid(), name: z.string().min(1).optional(), emoji: z.string().min(1).max(8).optional(), days: z.array(z.number().int().min(0).max(6)).optional() }),
  ...
  const deleteProgram = tool({
    description: "Supprimer un programme Sport appartenant à l'utilisateur.",
    inputSchema: z.object({ id: z.string().uuid() }),
  ...
  const programItem = tool({
    description: "Ajouter un exercice à un programme Sport, ou le retirer avec operation=remove.",
    inputSchema: z.object({ operation: z.enum(["add", "remove"]), programId: z.string().uuid(), exerciseId: z.string().uuid(), sets: z.number().int().positive().optional(), reps: z.number().int().positive().optional(), weight: z.number().nonnegative().optional(), restSec: z.number().int().nonnegative().optional(), position: z.number().int().nonnegative().optional() }),
  ...
  const startWorkout = tool({
    description: "Démarrer et persister une séance Sport.",
    inputSchema: z.object({ id: z.string().uuid().optional(), programId: z.string().uuid().optional(), name: z.string().min(1), workoutDate: z.string().regex(/^\\d{4}-\\d{2}-\\d{2}$/).optional(), startedAt: z.string().datetime().optional(), notes: z.string().optional() }),
  ...
  const addWorkoutExercise = tool({
    description: "Ajouter un exercice à une séance Sport persistée.",
    inputSchema: z.object({ sessionId: z.string().uuid(), exerciseId: z.string().uuid(), note: z.string().optional(), position: z.number().int().nonnegative().optional() }),
  ...
  const addWorkoutSet = tool({
    description: "Enregistrer une série avec répétitions, charge et état terminé.",
    inputSchema: z.object({ workoutExerciseId: z.string().uuid(), setNumber: z.number().int().positive(), reps: z.number().int().positive(), weight: z.number().nonnegative().default(0), done: z.boolean().default(true) }),
    execute: guard("add_workout_set", async (input) => {
      const { data: parent } = await client.from("sport_workout_exercises").select("id,session_id").eq("id", input.workoutExerciseId).maybeSingle();
      if (!parent) return { ok: false, message: "Exercice de séance introuvable." };
      const { data: session } = await client.from("sport_workout_sessions").select("id").eq("id", parent.session_id).eq("user_id", userId).maybeSingle();
      if (!session) return { ok: false, message: "Séance introuvable ou accès refusé." };
      const { data, error } = await client.from("sport_workout_sets").insert({ workout_exercise_id: input.workoutExerciseId, set_number: input.setNumber, reps: input.reps, weight: input.weight, done: input.done }).select("id").single();
      if (error || !data) { await logAction("add_workout_set", "Échec ajout série", input, "failed"); return { ok: false, message: "Enregistrement de la série impossible." }; }
      await logAction("add_workout_set", "Série enregistrée", { ...input, id: data.id }, "executed"); return { ok: true, id: data.id, message: "Série enregistrée." };
    }),
  });

  const updateWorkoutSet = tool({
    description: "Modifier une série existante d'une séance appartenant à l'utilisateur.",
    inputSchema: z.object({ id: z.string().uuid(), setNumber: z.number().int().positive().optional(), reps: z.number().int().positive().optional(), weight: z.number().nonnegative().optional(), done: z.boolean().optional() }),
    execute: guard("update_workout_set", async (input) => {
      const { data: set } = await client.from("sport_workout_sets").select("id,workout_exercise_id").eq("id", input.id).maybeSingle();
      if (!set) return { ok: false, message: "Série introuvable." };
      const { data: parent } = await client.from("sport_workout_exercises").select("session_id").eq("id", set.workout_exercise_id).maybeSingle();
      if (!parent) return { ok: false, message: "Série invalide." };
      const { data: session } = await client.from("sport_workout_sessions").select("id").eq("id", parent.session_id).eq("user_id", userId).maybeSingle();
      if (!session) return { ok: false, message: "Accès à la série refusé." };
      const { error } = await client.from("sport_workout_sets").update({ ...(input.setNumber === undefined ? {} : { set_number: input.setNumber }), ...(input.reps === undefined ? {} : { reps: input.reps }), ...(input.weight === undefined ? {} : { weight: input.weight }), ...(input.done === undefined ? {} : { done: input.done }) }).eq("id", input.id);
  ...
  const finishWorkout = tool({
    description: "Terminer une séance Sport persistée avec durée et notes éventuelles.",
    inputSchema: z.object({ id: z.string().uuid(), endedAt: z.string().datetime().optional(), durationMin: z.number().int().nonnegative().optional(), notes: z.string().optional() }),
    execute: guard("finish_workout", async (input) => {
      ...
      if (error || data !== true) { await logAction("finish_workout", "Échec fin séance", input, "failed"); return { ok: false, message: "Séance introuvable ou impossible à terminer." }; }
      await logAction("finish_workout", "Séance terminée", input, "executed"); return { ok: true, id: input.id, message: "Séance terminée et enregistrée." };
    }),
  });

  const getHistory = tool({
    description: "Lire l'historique des séances Sport de l'utilisateur, avec exercices et séries.",
    inputSchema: z.object({ limit: z.number().int().positive().max(100).default(20), fromDate: z.string().regex(/^\\d{4}-\\d{2}-\\d{2}$/).optional(), toDate: z.string().regex(/^\\d{4}-\\d{2}-\\d{2}$/).optional() }),
  ...
  const getProgress = tool({
    description: "Lire la progression d'un exercice Sport: dernières séries et meilleur poids/répétitions.",
    inputSchema: z.object({ exerciseId: z.string().uuid(), limit: z.number().int().positive().max(100).default(20) }),
  ...
  return {
    ...
    add_exercise_to_program: tool({ ...programItem, description: "Ajouter un exercice à un programme Sport." }),
    remove_exercise_from_program: tool({ ...programItem, description: "Retirer un exercice d'un programme Sport.", inputSchema: z.object({ operation: z.literal("remove").default("remove"), programId: z.string().uuid(), exerciseId: z.string().uuid() }) }),
