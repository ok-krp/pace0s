...
type CalendarEvent = { id: string; date: string; title: string; startTime?: string; endTime?: string; type?: string; notes?: string };
...
function CalendarPage() {
  ...
  const [calendarEvents] = useLocalState<CalendarEvent[]>("pace.calendar.events", []);
  ...
  const eventsForDay = (d: string) => calendarEvents.filter(event => event.date === d);
  const dayActivity = (d: string) => Number((sleep[d]?.hours ?? 0) > 0) + Number((water[d] ?? 0) > 0) + Number((routines[d] ?? []).length > 0) + Number((nutrition[d]?.kcal ?? 0) > 0) + Number(eventsForDay(d).length > 0);
  ...
  return <div>
    ...
    {selected && <DayDetails key={selected} date={selected} sleep={sleep[selected]} water={water[selected]} nutTotal={nutrition[selected]} nutList={nutItems[selected] ?? []} routineIds={routines[selected] ?? []} habits={habits} weight={weights[selected]} events={eventsForDay(selected)} />}
  </div>;
}

function DayDetails({ date, sleep, water, nutTotal, nutList, routineIds, habits, weight }: { date: string; sleep?: Sleep; water?: number; nutTotal?: { kcal?: number; p?: number; c?: number; f?: number }; nutList: NutItem[]; routineIds: string[]; habits: Habit[]; weight?: Weight; events: CalendarEvent[] }) {
  ...
  return <div className="mt-4 rounded-2xl glass-card p-5">
    ...
    <Accordion type="multiple" className="w-full">
      {events.length > 0 && <AccordionItem value="calendar"><AccordionTrigger><div className="flex items-center gap-2"><span className="size-2 bg-primary" /> Calendrier <span className="text-xs text-muted-foreground">({events.length} événement{events.length === 1 ? "" : "s"})</span></div></AccordionTrigger><AccordionContent><div className="space-y-2">{events.map(event => <div key={event.id} className="border-l border-primary/40 pl-3"><div className="text-sm font-medium">{event.title}</div><div className="text-xs text-muted-foreground">{event.startTime ? `${event.startTime}${event.endTime ? `–${event.endTime}` : ""}` : "Toute la journée"}{event.type ? ` · ${event.type}` : ""}</div>{event.notes && <div className="text-xs text-muted-foreground mt-1">{event.notes}</div>}</div>)}</div></AccordionContent></AccordionItem>}
