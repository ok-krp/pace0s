...
function validatePassword(password: string, email: string): string | null {
  ...
  const common = new Set([
    ...
    "1234567890", "qwerty", "qwerty123", "azerty", "azerty123", "admin", "admin123",
    "letmein", "welcome", "welcome1", "iloveyou", "monkey", "football", "dragon",
  ]);

  if (password.length < 8) return "Le mot de passe doit contenir au moins 8 caractères.";
  if (common.has(normalized)) return "Choisissez un mot de passe moins courant et plus difficile à deviner.";
  if (localPart && localPart.length >= 4 && normalized.includes(localPart)) {
    return "Évitez d'utiliser votre adresse email dans le mot de passe.";
  }

  const classes = [/[a-z]/.test(password), /[A-Z]/.test(password), /\d/.test(password), /[^A-Za-z\d]/.test(password)].filter(Boolean).length;
  if (classes < 3) return "Utilisez au moins 3 catégories : minuscules, majuscules, chiffres et symboles.";
  return null;
}
...
function LoginPage() {
    ...
    try {
      if (mode === "signup") {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: getRedirectUrl(target),
            data: { full_name: name },
          },
        });
  ...
  const handleGoogle = async () => {
    setBusy(true);
    try {
      const redirectTo = getRedirectUrl(target);
