...
function NutritionPage() {
  ...
  return (
    <div>
      ...
      <Link to="/recalls" className="mb-4 flex items-center gap-3 rounded-2xl glass-card p-4 hover:opacity-95 transition">
        <span className="glass-icon size-11 shrink-0 relative text-emerald-500">
          ...
          {recallCount > 0 && <span className="absolute -top-1 -right-1 size-4.5 rounded-full grid place-items-center bg-rose-500 text-white ring-1 ring-white/40 shadow-[0_2px_6px_-1px_rgba(0,0,0,0.4)]"><AlertTriangle className="size-2.5" /></span>}
        </span>
        <div className="min-w-0 flex-1"><div className="font-medium">Rappels conso</div><div className="text-xs text-muted-foreground">{recallCount > 0 ? `${recallCount} produit${recallCount > 1 ? "s" : ""} scanné${recallCount > 1 ? "s" : ""} concerné${recallCount > 1 ? "s" : ""} par un rappel` : "Aucun rappel actif sur tes produits scannés"}</div></div>
      ...
      <Dialog open={!!pending} onOpenChange={(v) => !v && setPending(null)}>
        {pending && <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle className="flex items-center gap-2">{pending.kind === "barcode" ? <ScanBarcode className="size-4" /> : <Camera className="size-4" />}{pending.kind === "barcode" ? pending.product.name : pending.result.dish_name}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            {pending.kind === "barcode" && pending.product.image_url && <img src={pending.product.image_url} alt={pending.product.name} className="size-24 rounded-xl object-cover mx-auto" />}
            {pending.kind === "barcode" ? <>
              <div className="text-xs text-muted-foreground text-center">{`Ref 100g : ${pending.product.kcal} kcal · P ${pending.product.protein_g} · G ${pending.product.carbs_g} · L ${pending.product.fat_g}`}</div>
              <div><label className="text-[11px] text-muted-foreground">Quantité (g)</label><Input type="number" inputMode="numeric" min={1} value={pending.grams} onChange={(e) => setPending({ ...pending, grams: Math.max(0, +e.target.value || 0) })} /></div>
            </> : <FoodAnalysisEditor items={pending.items} onChange={(items) => setPending({ ...pending, items, grams: sumItems(items).grams })} confidence={pending.result.confidence} confidenceNote={pending.result.confidence_note || pending.result.notes} />}
            <div><label className="text-[11px] text-muted-foreground">Repas</label><Select value={pending.meal} onValueChange={(v) => setPending({ ...pending, meal: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{MEALS.map((m) => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select></div>
            <div className="flex justify-end gap-2 pt-2"><Button variant="ghost" onClick={() => setPending(null)}><X className="size-4 mr-1" />Annuler</Button><Button onClick={confirmAdd} disabled={pending.kind === "barcode" ? !pending.grams : pending.items.length === 0}><Plus className="size-4 mr-1" />Ajouter</Button></div>
          </div>
        </DialogContent>}
      </Dialog>
    </div>
...
function NutritionLogView() {
  const [items, setItems] = useDomainState<Record<string, Item[]>>("nutrition.items", {});
  const [totals, setTotals] = useDomainState<Record<string, { kcal: number; p: number; c: number; f: number }>>("nutrition.totals", {});
  ...
  const [fields, setFields] = useState<Record<NutCol, string>>(Object.fromEntries(NUT_COLS.map((c) => [c.key, ""])) as Record<NutCol, string>);
  ...
  const add = () => {
    ...
    const next = { ...items, [today]: [...(items[today] ?? []), it] };
    setItems(next);
    setTotals({ ...totals, [today]: recompute(next[today]) });
    setName("");
    setFields(Object.fromEntries(NUT_COLS.map((c) => [c.key, ""])) as Record<NutCol, string>);
  };

  const remove = (id: string) => {
    const next = { ...items, [today]: (items[today] ?? []).filter((x) => x.id !== id) };
    setItems(next);
    setTotals({ ...totals, [today]: recompute(next[today]) });
  };
  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl glass-card p-5 mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">{activeCols.map((k) => { const meta = NUT_COLS.find((c) => c.key === k)!; return <Input key={k} placeholder={`${meta.label} (${meta.unit})`} type="number" value={fields[k]} onChange={(e) => setField(k, e.target.value)} />; })}</div>
        <div className="flex justify-between items-center mt-3 flex-wrap gap-2"><div className="text-xs text-muted-foreground">Objectif : {goal} kcal <span className="opacity-60">(profil)</span> · {activeCols.length} colonnes</div><Button onClick={add} className="rounded-xl"><Plus className="size-4 mr-1" />Ajouter</Button></div>
      </motion.div>
      <div className="space-y-4 pb-24 md:pb-4">
        {meals.map((m) => { const sub = list.filter((x) => x.meal === m); if (sub.length === 0) return null; return (
          <div key={m} className="rounded-2xl glass-card overflow-hidden">
            <div className="px-5 py-3 border-b border-border flex justify-between items-center bg-muted/30 flex-wrap gap-2"><div className="font-medium">{m}</div><div className="text-xs text-muted-foreground flex flex-wrap gap-3">{activeCols.map((k) => { const meta = NUT_COLS.find((c) => c.key === k)!; return <span key={k}>{Math.round(sumCol(sub, k))} {meta.unit}</span>; })}</div></div>
            <ul className="divide-y divide-border">{sub.map((x) => (
              <li key={x.id} className="px-5 py-3 flex justify-between items-center group gap-3"><div className="min-w-0"><div className="text-sm font-medium truncate">{x.name}</div><div className="text-xs text-muted-foreground truncate">{activeCols.filter((k) => k !== "kcal").map((k) => { const meta = NUT_COLS.find((c) => c.key === k)!; const v = Number(x[COL_FIELD[k]] as number | undefined) || 0; return `${meta.label.split(" ")[0]} ${v.toFixed(1)}${meta.unit}`; }).join(" · ")}</div></div><div className="flex items-center gap-3 shrink-0"><span className="text-sm font-medium">{x.kcal} kcal</span><button onClick={() => remove(x.id)} aria-label={`Supprimer ${x.name}`} className="text-muted-foreground hover:text-destructive opacity-60 group-hover:opacity-100 transition"><Trash2 className="size-4" /></button></div></li>
            ))}</ul>
          </div>
        ); })}
        {list.length === 0 && <div className="text-center py-12 text-muted-foreground text-sm">Aucun aliment aujourd'hui. Ajoute un repas ou scanne un code-barres via les boutons en bas.</div>}
      </div>
    </div>
  );
}
