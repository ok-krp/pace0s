...
import { lastNDays, fmtDay, todayKey } from "@/lib/storage";
...
function SleepPage() {
  ...
  // Sleep is a single-owned domain. Other surfaces must navigate here instead
  // of maintaining a second editable sleep value that can overwrite this one.
  useEffect(() => {
    const stored = entries[today];
    if (!stored) return;
    setStart((current) => current === stored.start ? current : stored.start);
    setEnd((current) => current === stored.end ? current : stored.end);
    setQuality((current) => current === stored.quality ? current : stored.quality);
  }, [entries, today]);
    ...
    <div>
      ...
      <div className="rounded-2xl glass-card p-5 mb-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div><label className="text-xs text-muted-foreground">Qualité (1-10)</label><NumberField allowDecimal={false} min={1} max={10} value={quality} onChange={(v) => { if (v != null) setQuality(v); }} className="w-24" /></div>
