...
as $$
declare
  item jsonb; program_item jsonb; session_ex jsonb; set_item jsonb;
  ex_id uuid; prog_id uuid; sess_id uuid; we_id uuid;
  v_user uuid := new.user_id;
begin
  if new.key = 'pace.sport.exercises' then
    delete from public.sport_exercises where user_id = v_user;
    ...
    delete from public.sport_program_items where program_id in (select id from public.sport_programs where user_id=v_user);
    delete from public.sport_programs where user_id=v_user;
    if jsonb_typeof(new.value) = 'array' then
      for item in select value from jsonb_array_elements(new.value) loop
