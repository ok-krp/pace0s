-- Gate health E2EE storage and device metadata on the active health/cloud-sync consents.
-- The function is intentionally SECURITY INVOKER so consent RLS remains authoritative.
    ...
    exists (
      ...
      where c.user_id = (select auth.uid())
        ...
        and c.created_at = (
          ...
          from public.consent_records c2
          where c2.user_id = (select auth.uid())
            and c2.consent_type = 'health_data'
        )
    )
    and exists (
      select 1
      ...
      where c.user_id = (select auth.uid())
        ...
        and c.created_at = (
          ...
          from public.consent_records c2
          where c2.user_id = (select auth.uid())
            and c2.consent_type = 'health_cloud_sync'
        )
    );
$$;
...
using (
  ...
  and public.has_current_health_e2ee_consent()
);
