...
begin
  ...
  for record in select value from jsonb_array_elements(p_records) loop
    ...
    if not exists (
      ...
      insert into public.health_samples_e2ee(user_id,ciphertext,nonce,algorithm,key_version,dedupe_hash)
      values (auth.uid(),record->>'ciphertext',record->>'nonce','AES-256-GCM',(record->>'key_version')::integer,dedupe_hash_value);
