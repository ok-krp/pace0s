...
begin
  ...
  if jsonb_typeof(p_updates) <> 'array' then raise exception 'Invalid backfill payload'; end if;
  if jsonb_array_length(p_updates) > 500 then raise exception 'Backfill batch too large'; end if;

  for item in select value from jsonb_array_elements(p_updates) loop
    row_id := (item->>'id')::uuid;
    hash_value := item->>'dedupe_hash';
    if hash_value is null or hash_value !~ '^[0-9a-f]{64}$' then raise exception 'Invalid dedupe hash'; end if;
    ...
    where target.id = row_id
      ...
      and not exists (
        select 1 from public.health_samples_e2ee as existing
